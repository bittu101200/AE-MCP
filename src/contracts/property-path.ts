...
export const PropertyPathSegmentSchema = z.object({
  ...
  (segment) => Boolean(segment.name || segment.matchName || segment.index),
  { message: "Property path segment requires name, matchName, or index." },
);
...
export type PropertyPathSegment = z.infer<typeof PropertyPathSegmentSchema>;
...
export function stringifyPropertyPath(path: PropertyPath): string {
  return path.segments
    .map((segment) => {
      if (segment.matchName) return `matchName:${segment.matchName}`;
      if (segment.name) return `name:${segment.name}`;
      return `index:${segment.index}`;
    })
    .join(" > ");
}

export function parsePropertyPath(input: string | PropertyPath): PropertyPath {
...
export function toBridgePropertyPath(input: string | PropertyPath): PropertyPath {
