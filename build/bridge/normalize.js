import { AE_ERROR_CODES } from "../contracts/ae-response.js";
function inferErrorCode(message) {
    if (/composition not found|comp not found/i.test(message))
        return AE_ERROR_CODES.COMP_NOT_FOUND;
    if (/layer not found/i.test(message))
        return AE_ERROR_CODES.LAYER_NOT_FOUND;
    if (/property .*not found|prop not found/i.test(message))
        return AE_ERROR_CODES.PROP_NOT_FOUND;
    if (/item not found/i.test(message))
        return AE_ERROR_CODES.ITEM_NOT_FOUND;
    if (/timed out/i.test(message))
        return AE_ERROR_CODES.TIMEOUT;
    if (/unsupported/i.test(message))
        return AE_ERROR_CODES.UNSUPPORTED_OPERATION;
    if (/invalid|out of range|no active comp|no project/i.test(message))
        return AE_ERROR_CODES.INVALID_VALUE;
    return AE_ERROR_CODES.INTERNAL_ERROR;
}
function isNormalizedEnvelope(parsed) {
    if (!parsed || typeof parsed !== "object")
        return false;
    const obj = parsed;
    return (obj.status === "success" || obj.status === "error") && typeof obj.command === "string" && typeof obj.timestamp === "string";
}
function stripDoubleWrapping(parsed) {
    if (parsed.data && typeof parsed.data === "object") {
        const inner = parsed.data;
        if (isNormalizedEnvelope(inner)) {
            return inner;
        }
    }
    return parsed;
}
export function createBridgeTimeoutResponse(command, commandId) {
    return {
        status: "error",
        command,
        commandId,
...
export function normalizeBridgePayload(text, command, commandId) {
    try {
        const parsed = JSON.parse(text);
        if (isNormalizedEnvelope(parsed)) {
            const stripped = stripDoubleWrapping(parsed);
            if (stripped !== parsed) {
                return stripped;
            }
            return parsed;
