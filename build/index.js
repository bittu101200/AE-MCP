...
async function waitForBridgeResult(commandId, expectedCommand, timeoutMs = 30000, pollMs = 100) {
...
function writeCommandFile(command, args = {}, commandId) {
...
async function queueAndWait(command, args, description, timeoutMs = 30000) {
    try {
        clearResultsFile();
        ...
        return {
            content: [{ type: "text", text: `Error executing ${description}: ${String(error)}` }],
            isError: true
        };
    }
}
...
function isBridgeErrorPayload(text) {
    try {
...
function listUnknownKeys(params, allowedKeys) {
    const allowed = new Set(allowedKeys);
    return Object.keys(params).filter((key) => !allowed.has(key));
}
function withNormalizedPropertyPath(params) {
...
const RUN_SCRIPT_PARAMETER_ALLOWLIST = {
...
function extFromUrlPath(pathname) {
    const ext = path.extname(pathname).toLowerCase();
    if (ext)
        return ext;
    return ".bin";
}
async function downloadToTempFile(urlString) {
...
// ─────────────────────────────────────────────────────────────
server.tool("run-script", "Run a predefined script in After Effects. For most operations, prefer using the dedicated tools (moveLayer, deleteLayer, etc.).", {
...
// TOOL: get-help
// ─────────────────────────────────────────────────────────────
server.tool("bridge-health", "Report file bridge status, last result timestamp, and result preview.", {}, async () => {
...
server.prompt("analyze-composition", { compositionName: z.string().describe("Name of the composition to analyze") }, (args) => ({
    messages: [{
            role: "user",
            content: {
                type: "text",
...
// ─────────────────────────────────────────────────────────────
server.tool("create-composition", "Create a new composition in After Effects with specified parameters", {
...
// ─────────────────────────────────────────────────────────────
// TOOL: moveLayer
server.tool("move-layer", "Reorder a layer in the composition timeline. Moves layer to a new index position.", {
...
server.tool("moveLayer", "Compatibility alias for move-layer.", {
    ...LayerIdentifierSchema,
    newIndex: z.number().int().positive().describe("Desired 1-based position in the layer stack.")
}, async (params) => queueAndWait("moveLayer", params, `move layer to index ${params.newIndex}`));
// TOOL: renameLayer
server.tool("rename-layer", "Rename a layer in the composition timeline.", {
    ...LayerIdentifierSchema,
...
server.tool("renameLayer", "Compatibility alias for rename-layer.", {
    ...LayerIdentifierSchema,
    newName: z.string().min(1).describe("New name for the layer.")
}, async (params) => queueAndWait("renameLayer", params, `rename layer to ${params.newName}`));
// TOOL: setLayerVisibility
server.tool("set-layer-visibility", "Toggle layer visibility (enabled/disabled) in the composition.", {
    ...LayerIdentifierSchema,
...
server.tool("setLayerVisibility", "Compatibility alias for set-layer-visibility.", {
    ...LayerIdentifierSchema,
    enabled: z.boolean().describe("Whether the layer should be visible (true) or hidden (false).")
}, async (params) => queueAndWait("setLayerVisibility", params, `set layer visibility to ${params.enabled}`));
// TOOL: deleteLayer
server.tool("delete-layer", "Delete (remove) a layer from the composition.", {
    ...LayerIdentifierSchema
}, async (params) => queueAndWait("deleteLayer", params, `delete layer`));
server.tool("deleteLayer", "Compatibility alias for delete-layer.", {
    ...LayerIdentifierSchema
}, async (params) => queueAndWait("deleteLayer", params, "delete layer"));
// TOOL: setBlendingMode
server.tool("set-blending-mode", "Set the blending mode of a layer (Normal, Add, Multiply, Screen, Overlay, etc.).", {
    ...LayerIdentifierSchema,
    blendMode: BlendModeSchema
}, async (params) => queueAndWait("setBlendingMode", params, `set blending mode to ${params.blendMode}`));
server.tool("setBlendingMode", "Compatibility alias for set-blending-mode.", {
    ...LayerIdentifierSchema,
    blendMode: BlendModeSchema
}, async (params) => queueAndWait("setBlendingMode", params, `set blend mode to ${params.blendMode}`));
// TOOL: setLayerParent
server.tool("set-layer-parent", "Set or remove parent-child relationship between layers. Parent layer controls child transforms.", {
    ...LayerIdentifierSchema,
...
server.tool("setLayerParent", "Compatibility alias for set-layer-parent.", {
    ...LayerIdentifierSchema,
    parentLayerIndex: z.number().int().positive().nullable().describe("1-based index of the parent layer (null to remove parenting)."),
    parentLayerName: z.string().optional().describe("Name of the parent layer (alternative to parentLayerIndex).")
}, async (params) => queueAndWait("setLayerParent", params, "set layer parent"));
// TOOL: createNullObject
server.tool("create-null-object", "Create a null object layer in the composition. Null objects are invisible layers used as controllers for parented animations.", {
    ...CompIdentifierSchema,
...
server.tool("createNullObject", "Compatibility alias for create-null-object.", {
    ...CompIdentifierSchema,
    name: z.string().optional().describe("Name for the null object (default: auto-generated).")
}, async (params) => queueAndWait("createNullObject", params, `create null object "${params.name || 'Null'}"`));
// TOOL: createAdjustmentLayer
server.tool("create-adjustment-layer", "Create an adjustment layer in the composition. Effects on adjustment layers apply to all layers below.", {
    ...CompIdentifierSchema,
...
server.tool("createAdjustmentLayer", "Compatibility alias for create-adjustment-layer.", {
    ...CompIdentifierSchema,
    name: z.string().optional().describe("Name for the adjustment layer (default: 'Adjustment Layer').")
}, async (params) => queueAndWait("createAdjustmentLayer", params, `create adjustment layer "${params.name || 'Adjustment Layer'}"`));
// ─────────────────────────────────────────────────────────────
...
server.tool("set-layer-keyframe", "Set a keyframe for a specific layer property at a given time.", {
    ...LayerIdentifierSchema,
...
server.tool("setLayerKeyframe", "Compatibility alias for set-layer-keyframe.", {
    ...LayerIdentifierSchema,
    propertyName: z.string().describe("Name of the property to keyframe (e.g., 'Position', 'Scale', 'Rotation', 'Opacity')."),
    timeInSeconds: z.number().describe("The time (in seconds) for the keyframe."),
    value: z.any().optional().describe("The value for the keyframe (e.g., [x,y] for Position, percentage for Opacity)."),
...
server.tool("set-layer-expression", "Set or remove an expression for a specific layer property.", {
    ...LayerIdentifierSchema,
...
server.tool("setLayerExpression", "Compatibility alias for set-layer-expression.", {
    ...LayerIdentifierSchema,
    propertyName: z.string().describe("Name of the property (e.g., 'Position', 'Scale', 'Rotation', 'Opacity')."),
    expressionString: z.string().describe("The expression string. Empty string removes the expression.")
}, async (params) => queueAndWait("setLayerExpression", params, `set expression on ${params.propertyName}`));
// TOOL: apply-effect
server.tool("apply-effect", "Apply an effect to a layer in After Effects.", {
    ...LayerIdentifierSchema,
...
server.tool("apply-effect-template", "Apply a predefined effect template to a layer in After Effects.", {
    ...LayerIdentifierSchema,
    templateName: z.enum([
...
server.tool("get-effects-help", "Get a reference of common After Effects effect matchNames and available templates.", {}, async () => {
...
server.tool("run-bridge-test", "Run bridge test effects to verify communication and apply test effects.", {}, async () => queueAndWait("bridgeTestEffects", {}, "bridge test effects"));
// ─────────────────────────────────────────────────────────────
// PRIORITY 2 TOOLS — Effect and Property Control
// ─────────────────────────────────────────────────────────────
// TOOL: getLayerEffects
server.tool("getLayerEffects", "List all effects currently applied to a layer, with index, name, matchName, and enabled state.", { ...LayerIdentifierSchema }, async (params) => queueAndWait("getLayerEffects", params, "get layer effects"));
// TOOL: getEffectProperties
server.tool("getEffectProperties", "Read all property names and current values for a specific effect on a layer.", {
    ...LayerIdentifierSchema,
...
server.tool("setEffectProperty", "Modify a property value on an existing effect applied to a layer.", {
    ...LayerIdentifierSchema,
...
// TOOL: removeEffect
server.tool("removeEffect", "Remove an effect from a layer.", {
    ...LayerIdentifierSchema,
...
server.tool("getLayerInfo", "Get comprehensive information about all layers in a composition, including position, scale, rotation, opacity, blending mode, parent, effects, 3D status, text content, and more. Supports filtering by type or specific layer indices.", {
    ...CompIdentifierSchema,
...
server.tool("inspect-property-tree", "Traverse nested After Effects property groups and return canonical property paths for follow-up tools.", {
    ...LayerIdentifierSchema,
...
server.tool("list-project-items", "List project panel items with IDs, hierarchy, and optional footage details.", {
...
server.tool("get-project-item-info", "Read detailed information for a single project item by ID, index, or name.", {
    ...ItemIdentifierSchema
}, async (params) => queueAndWait("getProjectItemInfo", params, "get project item info", 30000));
server.tool("get-composition-settings", "Read composition-level settings including dimensions, frame rate, duration, work area, and background color.", {
    ...CompIdentifierSchema
}, async (params) => queueAndWait("getCompositionSettings", params, "get composition settings", 30000));
server.tool("get-property-metadata", "Read property capabilities, dimensions, keyframe count, and expression support for a target property.", {
    ...LayerIdentifierSchema,
    ...PropertyTargetSchema
}, async (params) => queueAndWait("getPropertyMetadata", withNormalizedPropertyPath(params), "get property metadata", 30000));
server.tool("get-property-value", "Read the current or sampled value of a layer property using canonical property paths.", {
    ...LayerIdentifierSchema,
    ...PropertyTargetSchema,
...
server.tool("set-property-value", "Set a layer property directly using a canonical property path or simple property name.", {
    ...LayerIdentifierSchema,
    ...PropertyTargetSchema,
...
server.tool("get-expression", "Read the current expression, enabled state, and expression error for a property.", {
    ...LayerIdentifierSchema,
    ...PropertyTargetSchema
...
server.tool("search-text-layers", "Search all text layers in a composition for matching content. Returns layer index, name, text, and timing.", {
    ...CompIdentifierSchema,
...
server.tool("searchTextLayers", "Compatibility alias for search-text-layers.", {
    ...CompIdentifierSchema,
    searchText: z.string().describe("Text to search for in text layers."),
    caseSensitive: z.boolean().optional().default(false).describe("Whether search is case-sensitive (default: false).")
}, async (params) => queueAndWait("searchTextLayers", params, "search text layers", 30000));
...
// ─────────────────────────────────────────────────────────────
// TOOL: getKeyframes
server.tool("getKeyframes", "Read all existing keyframes on a layer property, including time, value, and interpolation type.", {
    ...LayerIdentifierSchema,
...
// TOOL: removeKeyframe
server.tool("removeKeyframe", "Remove a keyframe from a layer property by index or time.", {
    ...LayerIdentifierSchema,
...
// ─────────────────────────────────────────────────────────────
// TOOL: addMask
server.tool("addMask", "Add a mask to a layer. Supports rectangle, ellipse, or custom path shapes.", {
    ...LayerIdentifierSchema,
...
// TOOL: setTrackMatte
server.tool("setTrackMatte", "Set the track matte type for a layer (Alpha, Luma, etc.).", {
    ...LayerIdentifierSchema,
...
server.tool("import-file", "Import a file (footage, image, audio) into the After Effects project.", {
...
server.tool("importFile", "Compatibility alias for import-file.", {
    filePath: z.string().describe("Absolute path to the file to import."),
    importAs: z.enum(["footage", "composition", "project"]).optional().describe("How to import the file (default: footage).")
}, async (params) => queueAndWait("importFile", params, "import file"));
server.tool("import-from-url", "Download a remote file and import it into the After Effects project.", {
...
server.tool("importFromUrl", "Compatibility alias for import-from-url.", {
    url: z.string().url().describe("HTTP/HTTPS URL of the asset to download."),
    importAs: z.enum(["footage", "composition", "project"]).optional().describe("How to import the file (default: footage).")
}, async (params) => {
    try {
...
server.tool("add-layer-from-item", "Add an existing project item (footage, precomp, image) to a composition as a new layer.", {
    ...CompIdentifierSchema,
...
server.tool("addLayerFromItem", "Compatibility alias for add-layer-from-item.", {
    ...CompIdentifierSchema,
    itemId: z.number().int().positive().describe("Project item ID to add as a layer."),
    startTime: z.number().optional().describe("Start time for the layer (seconds)."),
    atIndex: z.number().int().positive().optional().describe("Position in the layer stack.")
...
server.tool("precompose-layers", "Precompose selected layers into a new composition.", {
    ...CompIdentifierSchema,
...
server.tool("precomposeLayers", "Compatibility alias for precompose-layers.", {
    ...CompIdentifierSchema,
    layerIndices: z.array(z.number().int().positive()).describe("Array of 1-based layer indices to precompose."),
    name: z.string().describe("Name for the new precomp."),
    moveAttributes: z.boolean().optional().describe("Move all attributes into new comp (default: true).")
...
server.tool("duplicate-layer", "Duplicate an existing layer in the composition.", { ...LayerIdentifierSchema }, async (params) => queueAndWait("duplicateLayer", params, "duplicate layer"));
// TOOL: duplicateLayer (compatibility)
server.tool("duplicateLayer", "Compatibility alias for duplicate-layer.", { ...LayerIdentifierSchema }, async (params) => queueAndWait("duplicateLayer", params, "duplicate layer"));
server.tool("get-active-comp", "Get the currently active composition in After Effects.", {}, async () => queueAndWait("getActiveComp", {}, "get active composition", 12000));
server.tool("getActiveComp", "Compatibility alias for get-active-comp.", {}, async () => queueAndWait("getActiveComp", {}, "get active composition", 12000));
server.tool("save-project", "Save the current After Effects project. Use saveAsPath to save to a new location.", {
...
server.tool("saveProject", "Compatibility alias for save-project.", {
    saveAsPath: z.string().optional().describe("Absolute path to save as a .aep file.")
}, async (params) => queueAndWait("saveProject", params, "save project"));
server.tool("open-project", "Open an After Effects project file. Requires closeCurrent=true when a project is already open.", {
...
server.tool("openProject", "Compatibility alias for open-project.", {
    filePath: z.string().describe("Absolute path to an .aep file."),
    closeCurrent: z.boolean().optional().describe("Required true to close current project context before opening another.")
}, async (params) => queueAndWait("openProject", params, "open project", 15000));
// ─────────────────────────────────────────────────────────────
...
server.tool("add-to-render-queue", "Add a composition to the After Effects render queue.", {
    ...CompIdentifierSchema,
...
server.tool("addToRenderQueue", "Compatibility alias for add-to-render-queue.", {
    ...CompIdentifierSchema,
    outputPath: z.string().optional().describe("Output file path for the render."),
    format: z.enum(["mp4", "mov", "avi", "png_sequence", "jpg_sequence"]).optional().describe("Output format.")
}, async (params) => queueAndWait("addToRenderQueue", params, "add to render queue"));
// TOOL: start-render (canonical)
server.tool("start-render", "Start rendering all items in the After Effects render queue.", {}, async () => queueAndWait("startRender", {}, "start render", 120000));
// TOOL: startRender (compatibility)
server.tool("startRender", "Compatibility alias for start-render.", {}, async () => queueAndWait("startRender", {}, "start render", 120000) // 2 min timeout for renders
);
// TOOL: capture-frame (canonical)
server.tool("capture-frame", "Capture a screenshot of the current composition at a specific time.", {
    ...CompIdentifierSchema,
...
server.tool("captureFrame", "Compatibility alias for capture-frame.", {
    ...CompIdentifierSchema,
    time: z.number().optional().describe("Time in seconds to capture (default: current time)."),
    outputPath: z.string().describe("Output file path for the screenshot."),
    format: z.enum(["png", "jpg"]).optional().describe("Image format (default: png).")
}, async (params) => queueAndWait("captureFrame", params, "capture frame"));
server.tool("render-composition", "Render one composition in one call (queue + render).", {
    ...CompIdentifierSchema,
...
server.tool("renderComposition", "Compatibility alias for render-composition.", {
    ...CompIdentifierSchema,
    outputPath: z.string().optional().describe("Optional output file path for render output."),
    format: z.enum(["mp4", "mov", "avi", "png_sequence", "jpg_sequence"]).optional().describe("Optional output format.")
}, async (params) => {
...
server.tool("execute-batch", "Execute multiple operations in a single call. Accepts kebab-case names publicly and camelCase for compatibility.", {
...
server.tool("executeBatch", "Compatibility alias for execute-batch.", {
    operations: z.array(z.object({
...
// TOOL: undo
server.tool("undo", "Undo the last operation(s) in After Effects.", {
    steps: z.number().int().positive().optional().describe("Number of undo steps (default: 1).")
}, async (params) => queueAndWait("undo", params || {}, "undo"));
// TOOL: redo
server.tool("redo", "Redo previously undone operation(s) in After Effects.", {
...
server.tool("setTextProperties", "Compatibility alias for set-text-properties.", {
    ...LayerIdentifierSchema,
...
server.tool("set-text-properties", "Set text document properties on a text layer: font, size, color, tracking, leading, justification, and text content.", {
    ...LayerIdentifierSchema,
    text: z.string().optional().describe("New text content."),
    fontFamily: z.string().optional().describe("Font family name (e.g., 'Arial', 'Helvetica Neue')."),
    fontSize: z.number().positive().optional().describe("Font size in pixels."),
...
server.tool("getTextProperties", "Compatibility alias for get-text-properties.", {
    ...LayerIdentifierSchema,
...
server.tool("get-text-properties", "Read all text document properties from a text layer: font, size, color, tracking, leading, justification, and text content. Supports batch mode via layerIndices.", {
    ...LayerIdentifierSchema,
    layerIndices: z.array(z.number().int().positive()).optional().describe("Get text from multiple layers at once. When provided, layerIndex/layerName are ignored.")
}, async (params) => queueAndWait("getTextProperties", params, "get text properties", 30000));
// 8.2 — Shape Layer Path Control
server.tool("modifyShapePath", "Modify the path of an existing shape group on a shape layer. Can update vertices, in/out tangents, and closed state.", {
    ...LayerIdentifierSchema,
...
server.tool("setShapeColors", "Set fill and/or stroke colors on an existing shape layer group.", {
    ...LayerIdentifierSchema,
    shapeGroupIndex: z.number().int().positive().optional().describe("1-based index of the shape group."),
    shapeGroupName: z.string().optional().describe("Name of the shape group."),
...
server.tool("addShapeGroup", "Add a new shape group (rectangle, ellipse, polygon, star, or custom path) to an existing shape layer.", {
    ...LayerIdentifierSchema,
...
server.tool("createCamera", "Create a camera layer in the composition with specified settings.", {
    ...CompIdentifierSchema,
...
server.tool("createLight", "Create a light layer in the composition.", {
    ...CompIdentifierSchema,
...
// 8.4 — Markers
server.tool("addMarker", "Add a marker to a layer or composition. Markers can have comments, chapter names, URLs, and duration.", {
    ...LayerIdentifierSchema,
...
server.tool("getMarkers", "Read all markers from a layer or composition, including time, comment, chapter, and duration.", {
    ...LayerIdentifierSchema,
...
server.tool("removeMarker", "Remove a marker from a layer or composition by index or nearest time.", {
    ...LayerIdentifierSchema,
...
// 8.5 — Time Remapping
server.tool("setTimeRemapping", "Enable/disable time remapping on a layer and optionally set time remap keyframes.", {
    ...LayerIdentifierSchema,
...
server.tool("setMotionBlur", "Toggle motion blur for a specific layer and/or the entire composition.", {
    ...LayerIdentifierSchema,
...
server.tool("setFrameBlending", "Set frame blending mode for a layer and/or enable at the composition level.", {
    ...LayerIdentifierSchema,
